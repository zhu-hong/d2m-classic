  ...
  return <Dialog open={open} maxWidth='1064px' onClose={onClose} scroll='paper'>
    ...
    <DialogContent className="w-1064px mt-24px">
      <div className="flex items-center text-2xl text-[#000c25]">
        ...
        <p className="mx-16px">{equipment.WorkstationName}</p>
          ...
          checkItem.map((c, i) => {
            return <Box key={c.OrdercaGuid} className={['w-full children:py-10px flex children:border-b not-last:children:border-r children:border-[#CECECE] children:pl-10px', ['bg-[#FFFFFF]','bg-[#F2F9F8]'][i%2]].join(' ')}>
              ...
              <Box sx={{width:'15%',padding:'0'}}>
                <input disabled={c.Type==='文本'} type="number" value={c.Value} onChange={(e) => changeByKey(c.OrdercaGuid, 'Value', e.target.value)} className="w-full h-full text-lg text-[#000c25] px-16px bg-transparent"></input>
              </Box>
              <Box sx={{width:'20%'}} className='flex items-center'>
                <IconButton disabled={c.Type==='数值'} onClick={() => changeByKey(c.OrdercaGuid, 'Result', '合格')}>
                  {
                    c.Result === '合格'
                    ?
                ...
                <IconButton disabled={c.Type==='数值'} onClick={() => changeByKey(c.OrdercaGuid, 'Result', '不合格')}>
                  {
                    c.Result === '不合格'
                    ?
